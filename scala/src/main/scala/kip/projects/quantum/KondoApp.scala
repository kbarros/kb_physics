...
object KondoApp extends App {
  val q = new Quantum(w=8, h=8, t=1, J_eff=2, e_min= -10, e_max= 10)  // hopping only: e_min= -6-0.5, e_max= 3+0.5
  ...
  val viz = new kip.md.Visualizer()
  viz.setBounds(Bounds3d(Vec3(0, 0, 0), Vec3(10, 10, 0)))
  viz.arrowHeadSize = 0.2
  ...
  def drawSpins() {
    val arrows = for (y <- 0 until q.h;
      ...
      val from = Vec3(x, y, 0)
      val to   = Vec3(x + sx.re, y + sy.re, sz.re)
      val normal = Vec3(0, 0, 1)
      val color = java.awt.Color.BLUE
      kip.md.Visualizer.Arrow(from, to, normal, color)
    }
    viz.arrows = arrows.toSeq
    viz.display()
  }
...
//  KPM.plotLines(cplot, (c.indices.toArray.map(i => (i+0.5)/c.size), c.toArray.map(math.abs(_))))
  ...
  for (iter <- 0 until 1000) {
    ...
    println("Exact: " + eig.filter(_ < e_cutoff).sum)
